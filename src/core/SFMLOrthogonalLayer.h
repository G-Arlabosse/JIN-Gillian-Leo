        ...
        for (auto y = yPos; y < yPos + chunkTileCount.y; ++y) {
          for (auto x = xPos; x < xPos + chunkTileCount.x; ++x) {
            if (idx < m_chunkTileIDs.size() &&
              ...
              sf::Vector2f tileIndex(idIndex % ca->tsTileCount.x,
  ...
  void update(sf::Time elapsed) {
    for (auto& c : m_visibleChunks) {
      for (AnimationState& as : c->getActiveAnimations()) {
        ...
        tile.flipFlags =
            0;  // TODO: get flipFlags from original tmx::TileLayer::Tile
        ...
        if (delta >
            overallDuration)  // loop the animation by resetting start time
        {
